...
void Implementation::Unfocus(IDataProvider* provider)
{
...

    {
        ...
        {
            const auto comp = renderData->activeComposition.write();
            comp->text.clear();
...
void Implementation::_doCompositionUpdate(TfEditCookie ec)
{
    ...
    // This code figures out the cursor position in the current document.
    // We currently don't need it because we don't render the cursor.
    // But if we ever need to, this code can be used as a reference.
#if 0
...
TextAttribute Implementation::_textAttributeFromAtom(TfGuidAtom atom) const
{
    ...
    // The IME will allow you to navigate between the 3 parts of the composition and the current one is marked as fBoldLine.
    if (da.fBoldLine)
    {
